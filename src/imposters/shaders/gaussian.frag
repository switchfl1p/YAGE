...
void main() {
    ...
    vec4 total_light = material.classic_color * ambient_intensity;

    for (int i = 0; i < point_light_count; i++) {
        total_light += calcPointLight(point_lights[i], surface_normal, view_dir);
    }

    for (int i = 0; i < dir_light_count; i++) {
        total_light += calcDirLight(dir_lights[i], surface_normal, view_dir);
    }
